...
import { AlertTriangle, Eye, PackagePlus, Pencil, Search } from 'lucide-react';
...
export default function StockItems() {
  ...
  const showAdd = () => { setForm(blank); setEditingId(null); setError(''); setOpen(true); };
  const showEdit = (item: StockItem) => {
    // opening_quantity is intentionally omitted: update_stock_item ignores it,
    // because stock levels move only through recorded movements.
    setForm({
      name: item.name, category: item.category, unit: item.unit, hsn_code: item.hsn_code ?? '',
      reorder_level: Number(item.reorder_level ?? 0), opening_quantity: 0,
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      if (editingId === null) await api.post('/stock-items', form);
      else await api.put(`/stock-items/${editingId}`, form);
      setOpen(false);
      await load();
    } catch (err: any) {
  ...
  return <>
    <PageHeader title="Stock Items" subtitle="Raw material and hardware inventory with reorder tracking" action={<Button onClick={showAdd}><PackagePlus size={15} /> Add Stock Item</Button>}  toolbar={<><div className="search-box"><Search size={16} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search item code, name or category..." /></div>
      <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14 }}><input type="checkbox" checked={lowStockOnly} onChange={e => setLowStockOnly(e.target.checked)} /> Low stock only</label></>}/>
        <Card className="list-card">{loading ? <Loading /> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Item Code</th><th>Item Name</th><th>Category</th><th>Unit</th><th>On Hand</th><th>Reorder Level</th><th>Status</th><th>Action</th></tr></thead><tbody>{pageRows.map(item => <tr key={item.id}>
      <td className="nowrap">{item.code}</td><td title={item.name}><b>{item.name}</b></td>
      <td>{item.category}</td>
      <td>{item.unit}</td>
      ...
      <td>{item.status}</td>
      <td><div className="action-group"><button className="mini-button" title="View movements" onClick={() => navigate(`/stock-items/${item.id}`)}><Eye size={14} /></button><button className="mini-button" title="Edit item" onClick={() => showEdit(item)}><Pencil size={14} /></button></div></td>
    </tr>)}
    {!filtered.length && <tr><td colSpan={8} className="muted">No stock items found</td></tr>}
    </tbody></table></div>}<Pagination {...pageProps} noun="stock items" /></Card>

    <Modal open={open} onClose={() => setOpen(false)} title={editingId === null ? 'Add Stock Item' : 'Edit Stock Item'} width={640}>
      <form onSubmit={submit}>
        <div className="form-grid">
          ...
          {editingId === null && <Field label="Opening Quantity"><Input type="number" min="0" step="0.01" value={form.opening_quantity} onChange={e => setForm({ ...form, opening_quantity: Number(e.target.value) })} /></Field>}
