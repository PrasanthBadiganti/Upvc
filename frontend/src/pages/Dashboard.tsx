  ...
  return (
    <>
      ...
      <div className="metric-grid dashboard-metrics">
        <MetricCard label="Total Leads" value={m.total_leads} change="12% vs Apr 2025" icon={UserPlus2} tone="blue" />
        <MetricCard label="Live Customers" value={m.live_customers} change="8% vs Apr 2025" icon={UsersRound} tone="teal" />
        <MetricCard label="Pending Customers" value={m.pending_customers} change="-6% vs Apr 2025" icon={BellRing} tone="amber" />
        <MetricCard label="Quotations This Month" value={m.quotations_this_month} change="15% vs Apr 2025" icon={FilePlus2} tone="blue" />
        <MetricCard label="Pending Payments" value={currency(m.pending_payments)} change="-5% vs Apr 2025" icon={WalletCards} tone="red" />
        <MetricCard label="Revenue Received" value={currency(m.revenue_received)} change="18% vs Apr 2025" icon={IndianRupee} tone="teal" />
      </div>
      ...
      <div className="dashboard-bottom">
        <Card className="compact-panel">
          <div className="card-head"><h3>Pending Payments</h3><button className="link-button">View All</button></div>
        ...
        <Card className="compact-panel">
          <div className="card-head"><h3>Today's Follow-ups</h3><button className="link-button">View All</button></div>
          <div className="followup-list">{data.today_followups.map((f, i) => <div className="followup-row" key={i}><div className="time-pill">{f.time}</div><div><b>{f.customer}</b><small className="cell-sub">{f.purpose}</small></div><Status value={f.channel} /></div>)}</div>
        </Card>
        <Card className="compact-panel">
          <div className="card-head"><h3>Recent Activity</h3><button className="link-button">View All</button></div>
          <div className="activity-list">{data.recent_activity.map((a, i) => <div className="activity-item" key={i}><div className="activity-icon">{a.type === 'payment' ? <IndianRupee size={16}/> : <ReceiptText size={16}/>}</div><div className="activity-copy"><b>{a.title}</b><small>{a.detail}</small></div><small>{a.when}</small></div>)}</div>
        </Card>
      </div>
    </>
  );
