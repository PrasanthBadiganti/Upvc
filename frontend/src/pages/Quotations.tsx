...
export default function Quotations() {
  ...
  return <>
    <PageHeader title="Quotations" subtitle="Create, track and convert quotations" action={<Link to="/quotations/new"><Button><FilePlus2 size={16}/> Create Quotation</Button></Link>}/>
    {location.state?.created && <div className="toast">Quotation {location.state.created} created successfully</div>}
    <div className="list-toolbar"><div className="search-box"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search quotation or customer..."/></div><Button tone="secondary" onClick={load}><RefreshCw size={15}/> Refresh</Button></div>
    <Card className="list-card">{loading?<Loading/>:<div className="table-wrap"><table className="data-table"><thead><tr><th>Quotation No.</th><th>Customer</th><th>Date</th><th>Validity</th><th>Items</th><th>Subtotal</th><th>GST</th><th>Grand Total</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.map(q=><tr key={q.id}><td><span className="cell-title">{q.number}</span><span className="cell-sub">{q.site_location}</span></td><td>{q.customer.name}</td><td>{shortDate(q.quotation_date)}</td><td>{q.validity_days} Days</td><td>{q.items.length}</td><td className="amount">{currency(q.subtotal,2)}</td><td className="amount">{currency(q.gst,2)}</td><td className="amount"><b>{currency(q.grand_total,2)}</b></td><td><Status value={q.status}/></td><td><div className="action-group"><button className="mini-button" onClick={()=>navigate(`/quotations/${q.id}`)}><Eye size={14}/></button><Button tone="ghost" onClick={()=>convert(q.id)} disabled={q.status==='Converted'}>Convert to Invoice</Button></div></td></tr>)}</tbody></table></div>}</Card>
  </>;
}
