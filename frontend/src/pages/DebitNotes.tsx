...
export default function DebitNotes(){
  ...
  const cancel=async(n:DebitNote)=>{if(!(await confirm({title:`Cancel ${n.number}?`,message:'The invoice balance will be reduced back.',confirmLabel:'Cancel note',cancelLabel:'Keep it',isDangerous:true})))return;await api.post(`/debit-notes/${n.id}/cancel`);load();};
  ...
  return <>
    <PageHeader title="Debit Notes" subtitle="Undercharge corrections issued against invoices" toolbar={<><div className="search-box"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search debit note, customer or invoice..."/></div><Button tone="secondary" onClick={load}><RefreshCw size={15}/> Refresh</Button></>}/>
    ...
    </tbody></table></div>}<Pagination {...pageProps} noun="debit notes" /></Card>
  </>;
}